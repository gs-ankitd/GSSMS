...
class Schedulesms extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('sms/schedulesms');
	...
	public function sendscheduledsms()
	{
      ...
      $number=$_POST['recipient_no'];
		...
		$date=$_POST['date'];
		$time=$_POST['time'];
		$delayuntil = date('d-m-Y H:i', strtotime($date.$time));
      ...
      $savedetails=$this->smsmaster_m->insert($smslogindata->text_username, $name,trim($number),$msg,$delayuntil,$status_msg,$reason,$batch=null,$type='Scheduled'); 
      
       redirect('Schedulesms', 'refresh'); 
	}
